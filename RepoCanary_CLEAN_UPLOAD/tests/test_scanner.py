...
class RepoCanaryTests(unittest.TestCase):
    ...
    def test_broken_markdown_link(self):
        ...
        result = scan_repository(root)
        self.assertTrue(any(x.check == "markdown-link" for x in result.findings))
    ...
    def test_moving_workflow_reference(self):
        ...
        result = scan_repository(root)
        self.assertTrue(any(x.check == "workflow-ref" for x in result.findings))
    ...
    def test_unconstrained_requirement(self):
        ...
        result = scan_repository(root)
        found = [x for x in result.findings if x.check == "python-requirement"]
